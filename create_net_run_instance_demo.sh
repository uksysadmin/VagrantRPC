...
ROUTER_ID=$(neutron router-list \
  | awk '/\ devops_router_1\ / {print $2}')

SUBNET_ID=$(neutron subnet-list \
  | awk '/\ devops_subnet_1\ / {print $2}')
...
NET_ID=$(neutron net-list | awk '/devops_network_1/ {print $2}')
...
neutron subnet-create --tenant-id ${TENANT_ID} --name devops_float_subnet_1 --allocation-pool start=192.168.100.10,end=192.168.100.20 --gateway 192.168.100.1 ext_net 192.168.100.0/24 --enable_dhcp=False
